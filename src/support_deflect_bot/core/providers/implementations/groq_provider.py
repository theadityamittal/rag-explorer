...
class GroqProvider(LLMProvider):
    ...
    def chat(self, 
        ...
        try:
            ...
            if 'rate limit' in error_str or 'quota' in error_str:
                raise ProviderRateLimitError(f"Groq rate limit exceeded: {e}", provider="groq", original_error=e)
            
            # Check for API errors
            if 'api' in error_str or 'unauthorized' in error_str:
                raise ProviderError(f"Groq API error: {e}", provider="groq", original_error=e)
            
            # Generic error
            raise ProviderError(f"Groq chat failed: {e}", provider="groq", original_error=e)
    ...
    def stream_chat(self, 
        ...
        try:
            ...
            raise ProviderError(f"Groq streaming failed: {e}", provider="groq", original_error=e)