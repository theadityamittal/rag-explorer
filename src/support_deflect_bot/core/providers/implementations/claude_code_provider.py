...
from ..base import (
    LLMProvider, ProviderConfig, ProviderType, ProviderTier,
...
class ClaudeCodeProvider(LLMProvider):
    ...
    def __init__(self, claude_path: Optional[str] = None, **kwargs):
        ...
        logger.info(f"Initialized Claude Code provider with path: {self.executable_path}")
    ...
    def chat(self, 
        ...
        """
        if not self.executable_path:
            raise ProviderUnavailableError("Claude Code executable not found", provider="claude_code")
        ...
        try:
            ...
            try:
                ...
                result = subprocess.run([
                    ...
                    '--file', temp_file_path,
                    '--mode', 'completion'
                ], capture_output=True, timeout=120, text=True)  # 2 minute timeout
                
                if result.returncode != 0:
                    error_msg = result.stderr.strip() if result.stderr else "Unknown error"
                    raise ProviderError(f"Claude Code failed: {error_msg}", provider="claude_code")
                
                response = result.stdout.strip()
                
                if not response:
                    raise ProviderError("Claude Code returned empty response", provider="claude_code")
            ...
            raise ProviderError("Claude Code timed out after 2 minutes", provider="claude_code")
        except Exception as e:
            if isinstance(e, (ProviderError, ProviderUnavailableError)):
                raise
            raise ProviderError(f"Claude Code execution failed: {e}", provider="claude_code", original_error=e)
    ...
    def get_usage_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'models_included': ['Claude 3 Sonnet', 'Claude 3 Haiku', 'Claude 3.5 Sonnet'],
