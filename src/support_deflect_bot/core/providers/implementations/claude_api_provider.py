...
class ClaudeAPIProvider(LLMProvider):
    ...
    def chat(self, 
        ...
        """
        if not self.client:
            raise ProviderUnavailableError("Claude API client not available", provider="claude_api")
        ...
        try:
            ...
            raise ProviderRateLimitError(f"Claude API rate limit exceeded: {e}", provider="claude_api", original_error=e)
        except self.anthropic.APIError as e:
            raise ProviderError(f"Claude API error: {e}", provider="claude_api", original_error=e)
        except Exception as e:
            raise ProviderError(f"Claude API chat failed: {e}", provider="claude_api", original_error=e)
