...
class MistralProvider(LLMProvider):
    ...
    def chat(self, 
        ...
        """
        if not self.client:
            raise ProviderUnavailableError("Mistral client not available", provider="mistral")
        ...
        try:
            ...
            if 'rate limit' in error_str or 'quota' in error_str or '429' in error_str:
                raise ProviderRateLimitError(f"Mistral rate limit exceeded: {e}", provider="mistral", original_error=e)
            
            # Check for API errors
            if 'api' in error_str or 'unauthorized' in error_str or '401' in error_str:
                raise ProviderError(f"Mistral API error: {e}", provider="mistral", original_error=e)
            
            # Check for model errors
            if 'model' in error_str and 'not found' in error_str:
                raise ProviderError(f"Mistral model not available: {model}", provider="mistral", original_error=e)
            
            # Generic error
            raise ProviderError(f"Mistral chat failed: {e}", provider="mistral", original_error=e)
    ...
    def stream_chat(self, 
        ...
        try:
            ...
            raise ProviderError(f"Mistral streaming failed: {e}", provider="mistral", original_error=e)
