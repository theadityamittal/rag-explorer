...
class BaseProvider(ABC):
    ...
    def is_region_supported(self, region: str) -> bool:
        ...
        return 'global' in self.config.regions_supported or region in self.config.regions_supported
    
    def calculate_cost(self, input_tokens: int, output_tokens: int = 0) -> float:
        """Calculate cost for given token usage."""
        input_cost = (input_tokens / 1_000_000) * self.config.cost_per_million_tokens_input
        output_cost = (output_tokens / 1_000_000) * self.config.cost_per_million_tokens_output
