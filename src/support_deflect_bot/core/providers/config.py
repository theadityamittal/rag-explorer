...
from .base import BaseProvider, LLMProvider, EmbeddingProvider, ProviderConfig, ProviderType, ProviderTier
...
class ProviderRegistry:
    ...
    def register_provider_class(self, 
                               name: str,
    ...
    def get_or_create_provider(self, 
        ...
        try:
            ...
            if not self._compliance_checker.check_provider_compliance(name, self._user_region):
                logger.warning(f"Provider {name} not compliant for region {self._user_region}")
                return None
    ...
    def get_available_providers(self, 
                              ...
                              region: Optional[str] = None) -> List[Tuple[str, BaseProvider]]:
        ...
        for name, provider_class in self._provider_classes.items():
            try:
                ...
                if provider:
                    ...
                    config = provider.get_config()
                    if provider_type is None or config.provider_type == provider_type or config.provider_type == ProviderType.BOTH:
        ...
        logger.info(f"Found {len(available)} available providers for type {provider_type} in region {region}")
        return available
    
    def build_fallback_chain(self, 
                           provider_type: ProviderType,
                           strategy: str = 'cost_optimized',
                           exclude: Optional[List[str]] = None) -> List[BaseProvider]:
        ...
        try:
            ...
            strategy_config = self._strategy_manager.get_strategy(StrategyType.COST_OPTIMIZED)
        
        # Get available providers
        available_providers = self.get_available_providers(provider_type, self._user_region)
        
        # Filter out excluded providers
        filtered_providers = [
            (name, provider) for name, provider in available_providers
        ...
        for name, provider in filtered_providers:
            ...
            if (config.cost_per_million_tokens_input > strategy_config.max_cost_per_million_input or
                config.cost_per_million_tokens_output > strategy_config.max_cost_per_million_output):
                continue
            ...
            if not strategy_config.prefer_free_tier and config.tier == ProviderTier.FREE:
                continue
            ...
            score = self._strategy_manager.calculate_provider_score(strategy_config, metrics)
        ...
        logger.info(f"Built fallback chain with {len(chain)} providers using strategy {strategy}")
    ...
    def _calculate_provider_metrics(self, config: ProviderConfig) -> Dict[str, float]:
        ...
        cost_input_score = max(0, 1 - (config.cost_per_million_tokens_input / max_reasonable_cost))
        cost_output_score = max(0, 1 - (config.cost_per_million_tokens_output / max_reasonable_cost))
        ...
        tier_scores = {ProviderTier.FREE: 0.3, ProviderTier.PAID: 0.7, ProviderTier.PREMIUM: 1.0}
...
class ProviderSelector:
    ...
    def select_primary_provider(self, 
                              provider_type: ProviderType,
                              strategy: str = 'cost_optimized',
                              region: Optional[str] = None) -> Optional[BaseProvider]:
    ...
    def select_with_fallbacks(self, 
                            provider_type: ProviderType,
                            strategy: str = 'cost_optimized',
                            max_fallbacks: int = 3) -> List[BaseProvider]:
...
def get_available_providers(provider_type: Optional[ProviderType] = None) -> List[Tuple[str, BaseProvider]]:
    """Get available providers from default registry."""
    registry = get_default_registry()
    return registry.get_available_providers(provider_type)


def build_fallback_chain(provider_type: ProviderType, 
                        strategy: str = 'cost_optimized') -> List[BaseProvider]:
