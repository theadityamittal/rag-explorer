...
class DocsConfig(BaseModel):
    ...
    sources: List[str] = Field(default_factory=lambda: ["./docs"], description="List of documentation sources")
...
class RagConfig(BaseModel):
    ...
    confidence_threshold: float = Field(0.25, ge=0.0, le=1.0, description="Minimum confidence threshold for answers")
    max_chunks: int = Field(5, ge=1, le=20, description="Maximum number of chunks to retrieve")
    max_chars_per_chunk: int = Field(800, ge=100, le=5000, description="Maximum characters per chunk")
...
class CrawlConfig(BaseModel):
    ...
    allow_hosts: List[str] = Field(
        default_factory=lambda: ["docs.python.org", "packaging.python.org", "pip.pypa.io"],
    ...
    max_pages: int = Field(40, ge=1, le=500, description="Maximum number of pages to crawl")
    ...
    def validate_trusted_domains_subset(self):
        ...
        if not trusted_domains.issubset(allow_hosts):
            ...
            raise ValueError(f"Trusted domains must be subset of allowed hosts. Extra domains: {extra_domains}")
...
class ModelOverridesConfig(BaseModel):
    ...
    gemini_llm_model: Optional[str] = Field(None, description="Override Gemini LLM model")
    gemini_embedding_model: Optional[str] = Field(None, description="Override Gemini embedding model")
    openai_llm_model: Optional[str] = Field(None, description="Override OpenAI LLM model")
    openai_embedding_model: Optional[str] = Field(None, description="Override OpenAI embedding model")
...
class AppConfig(BaseModel):
    ...
    # General settings
    primary_llm_provider: str = Field("google_gemini_paid", description="Primary LLM provider")
    primary_embedding_provider: str = Field("google_gemini_paid", description="Primary embedding provider")
    monthly_budget_usd: float = Field(10.0, ge=0.0, description="Monthly budget limit in USD")
