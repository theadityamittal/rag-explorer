...
from .schema import AppConfig, ApiKeysConfig, DocsConfig, RagConfig, CrawlConfig, ModelOverridesConfig
...
class ConfigurationManager:
    ...
    def export_env_file(self, file_path: str) -> None:
        ...
        try:
            with open(file_path, 'w') as f:
                ...
                f.write("# Generated automatically - edit with 'deflect-bot configure'\n\n")
    ...
    def validate_config(self) -> Dict[str, Any]:
        ...
        if not any(api_keys):
            warnings.append("No API keys configured. Add at least one API key for enhanced functionality.")
        
        # Check docs path
        if not os.path.exists(config.docs.local_path):
            warnings.append(f"Documentation path does not exist: {config.docs.local_path}")
        
        # Check RAG config
        if config.rag.confidence_threshold < 0.1:
            warnings.append("Very low confidence threshold may result in unreliable answers")
        elif config.rag.confidence_threshold > 0.9:
            warnings.append("Very high confidence threshold may result in too many refusals")
    ...
    def _load_from_environment(self) -> Dict[str, Any]:
        ...
        if os.getenv('ANSWER_MIN_CONF'):
            env_config["rag"]["confidence_threshold"] = float(os.getenv('ANSWER_MIN_CONF'))
        if os.getenv('MAX_CHUNKS'):
            env_config["rag"]["max_chunks"] = int(os.getenv('MAX_CHUNKS'))
        if os.getenv('MAX_CHARS_PER_CHUNK'):
            env_config["rag"]["max_chars_per_chunk"] = int(os.getenv('MAX_CHARS_PER_CHUNK'))
        
        # Crawl
        if os.getenv('ALLOW_HOSTS'):
            env_config["crawl"]["allow_hosts"] = [h.strip() for h in os.getenv('ALLOW_HOSTS').split(',')]
        if os.getenv('TRUSTED_DOMAINS'):
            env_config["crawl"]["trusted_domains"] = [d.strip() for d in os.getenv('TRUSTED_DOMAINS').split(',')]
        if os.getenv('DEFAULT_SEEDS'):
            env_config["crawl"]["default_seeds"] = [s.strip() for s in os.getenv('DEFAULT_SEEDS').split(',')]
        ...
        if os.getenv('CRAWL_SAME_DOMAIN'):
            env_config["crawl"]["same_domain"] = os.getenv('CRAWL_SAME_DOMAIN').lower() == 'true'
        if os.getenv('CRAWL_USER_AGENT'):
            env_config["crawl"]["user_agent"] = os.getenv('CRAWL_USER_AGENT')
        
        # Model overrides
        if os.getenv('GOOGLE_LLM_MODEL'):
            env_config["model_overrides"]["gemini_llm_model"] = os.getenv('GOOGLE_LLM_MODEL')
        if os.getenv('GOOGLE_EMBEDDING_MODEL'):
            env_config["model_overrides"]["gemini_embedding_model"] = os.getenv('GOOGLE_EMBEDDING_MODEL')
        if os.getenv('OPENAI_LLM_MODEL'):
            env_config["model_overrides"]["openai_llm_model"] = os.getenv('OPENAI_LLM_MODEL')
        if os.getenv('OPENAI_EMBEDDING_MODEL'):
            env_config["model_overrides"]["openai_embedding_model"] = os.getenv('OPENAI_EMBEDDING_MODEL')
        ...
        if os.getenv('PRIMARY_EMBEDDING_PROVIDER'):
            env_config["primary_embedding_provider"] = os.getenv('PRIMARY_EMBEDDING_PROVIDER')
