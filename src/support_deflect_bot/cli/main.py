...
def index(ctx, docs_path):
    ...
    try:
        ...
        if not ctx.obj["quiet"]:
            console.print(
                f"🔄 Indexing local documentation from {folder_path}...", style="cyan"
            )
        ...
        if ctx.obj["quiet"]:
            ...
            console.print(
                f"✅ Successfully indexed {n} chunks from {DOCS_FOLDER}", style="green"
            )
...
@click.option(
    "--confidence", type=float, help="Override confidence threshold (0.0-1.0)"
)
...
@click.option(
    "--seeds", help="Comma-separated list of seed URLs (alternative to positional URLs)"
)
@click.option(
    "--default", "use_default", is_flag=True, help="Use configured default seeds"
)
@click.pass_context
def crawl(
    ctx,
    ...
    try:
        if use_default:
            ...
            if not ctx.obj["quiet"]:
                console.print(
                    f"🕷️ Crawling default seeds: {', '.join(DEFAULT_SEEDS)}",
                    style="cyan",
                )
            ...
            if not ctx.obj["quiet"]:
                console.print(
                    f"🕷️ Deep crawling {len(url_list)} seeds (depth: {depth}, max: {max_pages})",
                    style="cyan",
                )
            ...
            console.print(
                "❌ Provide URLs, use --default, or specify crawl options", style="red"
            )
            raise click.Abort()
