...
def _configure_api_keys(config_data: dict):
    ...
    display_google = f"[green]✓ Configured[/green]" if current_google else "[red]✗ Not set[/red]"
    ...
    display_openai = f"[green]✓ Configured[/green]" if current_openai else "[red]✗ Not set[/red]"
    ...
    if Confirm.ask("Configure additional providers (Anthropic, Groq, Mistral)?", default=False):
        providers = [
        ...
        for key, label in providers:
            ...
            new_key = Prompt.ask(
                label,
                default=current[:10] + "..." if current else "",
                password=True
            )
...
def _configure_docs(config_data: dict):
    ...
    docs['auto_refresh'] = Confirm.ask("Auto-refresh documentation", default=auto_refresh)
...
def _configure_rag(config_data: dict):
    ...
    console.print("Lower values = more answers (less selective), Higher values = fewer answers (more selective)")
    
    while True:
        conf_input = Prompt.ask("Confidence threshold (0.0-1.0)", default=str(current_conf))
        try:
    ...
    while True:
        chunks_input = Prompt.ask("Maximum chunks to retrieve (1-20)", default=str(current_chunks))
        try:
    ...
    while True:
        chars_input = Prompt.ask("Maximum characters per chunk (100-5000)", default=str(current_chars))
        try:
...
def _configure_crawl(config_data: dict):
    ...
    trusted_input = Prompt.ask(
        "Trusted domains (subset of allowed hosts)",
        default=','.join(current_trusted)
    )
    crawl['trusted_domains'] = [d.strip() for d in trusted_input.split(',') if d.strip()]
    
    # Default seeds
    current_seeds = crawl.get('default_seeds', ['https://docs.python.org/3/faq/index.html'])
    ...
    while True:
        max_input = Prompt.ask("Maximum pages per crawl (1-500)", default=str(current_max))
        try:
    ...
    crawl['same_domain'] = Confirm.ask("Restrict crawling to same domain", default=current_same_domain)
...
def _show_configuration_summary(config: AppConfig):
    ...
    # API Keys
    table.add_row("🔑 Google API Key", "✓ Configured" if config.api_keys.google_api_key else "✗ Not set")
    table.add_row("🔑 OpenAI API Key", "✓ Configured" if config.api_keys.openai_api_key else "✗ Not set")
