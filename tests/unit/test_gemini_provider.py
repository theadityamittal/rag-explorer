...
class TestGoogleGeminiFreeProvider:
    ...
    def test_embed_texts_error_handling(self, mock_genai):
        ...
        assert len(result[1]) == 768 and all(
            x == 0.0 for x in result[1]
        )  # Error -> zero vector
    ...
    def test_count_tokens(self, mock_genai):
        ...
        mock_genai.GenerativeModel.return_value.count_tokens.return_value.total_tokens = (
            10
        )
        ...
        assert count == 10
        mock_genai.GenerativeModel.return_value.count_tokens.assert_called_once_with(
            "Hello world test"
        )
    ...
    def test_count_tokens_error(self, mock_genai):
        ...
        mock_genai.GenerativeModel.return_value.count_tokens.side_effect = Exception(
            "Count error"
        )
...
class TestEnvironmentVariableIntegration:
    ...
    def test_model_override_from_settings(self, mock_genai):
        ...
        with patch(
            "support_deflect_bot.core.providers.implementations.google_gemini.GOOGLE_LLM_MODEL",
            "custom-gemini-model",
        ):
            with patch(
                "support_deflect_bot.core.providers.implementations.google_gemini.GOOGLE_EMBEDDING_MODEL",
