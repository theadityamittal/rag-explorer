...
class TestApiKeysConfig:
    ...
    def test_empty_string_to_none(self):
        ...
        config = ApiKeysConfig(
            google_api_key="", openai_api_key="valid_key", anthropic_api_key=""
        )
...
class TestRagConfig:
    ...
    def test_valid_rag_config(self):
        ...
        config = RagConfig(
            confidence_threshold=0.75, max_chunks=10, max_chars_per_chunk=1200
        )
...
class TestModelOverridesConfig:
    ...
    def test_empty_string_to_none(self):
        """Test that empty strings are converted to None."""
        config = ModelOverridesConfig(
            gemini_llm_model="", openai_llm_model="gpt-4o", openai_embedding_model=""
        )
    ...
    def test_get_flat_dict(self):
        ...
        config = AppConfig(
            api_keys=ApiKeysConfig(
                google_api_key="test_google", openai_api_key="test_openai"
            ),
            docs=DocsConfig(local_path="/test/docs"),
            rag=RagConfig(confidence_threshold=0.6, max_chunks=8),
            crawl=CrawlConfig(
                allow_hosts=["site1.com", "site2.com"], depth=3, same_domain=False
            ),
