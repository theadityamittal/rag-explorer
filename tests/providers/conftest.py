...
def sample_providers(mock_provider):
    ...
    return {
        'cheap': mock_provider(
            name='cheap', 
            config_override={'cost_input': 0.15, 'cost_output': 0.60}
        ),
        'expensive': mock_provider(
            name='expensive',
            config_override={'cost_input': 10.0, 'cost_output': 30.0}
        ),
        'free': mock_provider(
            name='free',
        ...
        'gdpr_compliant': mock_provider(
            name='gdpr_compliant',
            config_override={'gdpr_compliant': True}
        ),
        'non_gdpr': mock_provider(
            name='non_gdpr',
            config_override={'gdpr_compliant': False}
        ),
...
def regional_compliance_setup():
    ...
    def _setup_region(region: str, gdpr_required: bool = None):
        ...
        with patch.dict(os.environ, config), \
             patch('src.support_deflect_bot.utils.region_detector.RegionDetector.detect_region', return_value=region), \
             patch('src.support_deflect_bot.utils.region_detector.is_gdpr_region', return_value=gdpr_required if gdpr_required is not None else region in ['DE', 'FR', 'IT', 'ES', 'NL']):
