...
class TestProviderRegistration:
    ...
    def test_all_providers_register(self):
        ...
        expected_providers = {
            'openai', 'groq', 'mistral', 
            'google_gemini_free', 'google_gemini_paid',
            'claude_api', 'claude_code', 'ollama'
        }
        
        registered = set(registry._provider_classes.keys())
        assert registered == expected_providers, f"Missing providers: {expected_providers - registered}"
    ...
    def test_provider_configs(self):
        ...
        for name, provider_class in registry._provider_classes.items():
            try:
                ...
                assert config.provider_type in [ProviderType.LLM, ProviderType.EMBEDDING, ProviderType.BOTH]
                assert config.tier in [ProviderTier.FREE, ProviderTier.PAID, ProviderTier.PREMIUM]
...
class TestFallbackChains:
    ...
    def test_cost_optimized_fallback_order(self):
        ...
        with patch.object(self.registry, 'get_available_providers') as mock_available:
            ...
            chain = self.registry.build_fallback_chain(ProviderType.LLM, 'cost_optimized')
...
class TestRegionalCompliance:
    ...
    def test_gdpr_compliance_filtering(self, mock_is_gdpr):
        ...
        with patch.object(self.registry._region_detector, 'detect_region', return_value='DE'):
            # Get available providers for EU region
            available = self.registry.get_available_providers(ProviderType.LLM, region='DE')
            
            # Check that only GDPR-compliant providers are included
            for name, provider in available:
                config = provider.get_config()
                if not config.gdpr_compliant:
                    pytest.fail(f"Non-GDPR compliant provider {name} included in EU region")
    ...
    def test_non_gdpr_region_all_providers(self, mock_is_gdpr):
        ...
        with patch.object(self.registry._region_detector, 'detect_region', return_value='US'):
            # Should include both GDPR and non-GDPR providers
            eu_available = self.registry.get_available_providers(ProviderType.LLM, region='DE')
            us_available = self.registry.get_available_providers(ProviderType.LLM, region='US')
...
class TestCostOptimization:
    ...
    def test_provider_cost_scoring(self):
        ...
        expensive_score = strategy_manager.calculate_provider_score(cost_strategy, expensive_metrics)
        ...
        cheap_score = strategy_manager.calculate_provider_score(cost_strategy, cheap_metrics)
        
        # Cost-optimized strategy should score cheap provider higher
        assert cheap_score > expensive_score, "Cost-optimized strategy should prefer cheaper providers"
    ...
    def test_provider_health_check_structure(self):
        ...
        for name, provider_class in registry._provider_classes.items():
            try:
                ...
                assert health['status'] in ['healthy', 'unhealthy', 'degraded', 'rate_limited', 'auth_error']
                assert health['provider'] == name.replace('_', '') or health['provider'] in name
...
class TestIntegration:
    ...
    def test_strategy_adaptation_for_region(self):
        ...
        assert eu_strategy.max_cost_per_million_input >= base_strategy.max_cost_per_million_input
...
class TestConfiguration:
    ...
    def test_settings_integration(self):
        ...
        assert OPENAI_API_KEY is not None or OPENAI_API_KEY is None  # Exists in namespace
        assert isinstance(MONTHLY_BUDGET_USD, float)
        assert isinstance(COST_ALERT_THRESHOLD, float)
