...
class TestAskEndpoint:
    ...
    def test_ask_with_domains(self, mock_answer):
        ...
        assert response.status_code == 200
        mock_answer.assert_called_once_with("Domain-specific question?", domains=["docs.example.com"])
    ...
    def test_ask_invalid_request(self):
        ...
        response = client.post("/ask", json={"question": "test", "domains": many_domains})
        assert response.status_code == 422
...
class TestBatchAskEndpoint:
    ...
    def test_batch_ask_success(self, mock_batch):
        ...
        client = TestClient(app)
        response = client.post("/batch_ask", json={
            "questions": ["How to install?", "How to configure?"]
        })
...
class TestCrawlEndpoints:
    ...
    def test_crawl_depth_success(self, mock_crawl):
        ...
        client = TestClient(app)
        response = client.post("/crawl_depth", json={
