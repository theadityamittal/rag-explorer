...
class TestMultiProviderIntegration:
    ...
    def setup_method(self):
        ...
        from src.support_deflect_bot.core.providers.implementations import register_all_providers
    ...
    def test_provider_registration_integration(self):
        ...
        required_providers = {
            'openai', 'groq', 'mistral', 'google_gemini_free', 
            'google_gemini_paid', 'claude_api', 'claude_code', 'ollama'
        }
    ...
    def test_fallback_chain_building_integration(self):
        ...
        for strategy in strategies:
            ...
            if len(chain) > 1:
                ...
                logger.info(f"Strategy '{strategy}': No available providers (expected without API keys)")
        ...
        for name, provider_class in registry._provider_classes.items():
            try:
                ...
                assert health['status'] in [
                    'healthy', 'unhealthy', 'degraded', 'rate_limited', 'auth_error'
                ]
    ...
    def test_provider_selector_integration(self):
        ...
        from src.support_deflect_bot.core.providers.config import get_default_registry, ProviderSelector
        from src.support_deflect_bot.core.providers.base import ProviderType
        ...
        fallbacks = selector.select_with_fallbacks(ProviderType.LLM, 'cost_optimized', max_fallbacks=3)
    ...
    def test_cost_optimization_integration(self):
        ...
        from src.support_deflect_bot.core.providers.strategies import StrategyManager, StrategyType
        ...
        for strategy_type in strategies:
            ...
            logger.info(f"{strategy_type.value}: Cost factor = {strategy.priority_factors.get('cost', 0)}")
    ...
    def test_settings_integration(self):
        ...
        try:
            from support_deflect_bot.utils.settings import (
                ...
                DEFAULT_STRATEGY, REGIONAL_COMPLIANCE,
                OPENAI_LLM_MODEL, CLAUDE_API_MODEL, GROQ_MODEL,
                MISTRAL_MODEL, GOOGLE_MODEL, CLAUDE_CODE_PATH
            )
            ...
            logger.info(f"Budget: ${MONTHLY_BUDGET_USD}/month, Alert: {COST_ALERT_THRESHOLD*100}%")
...
class TestRegionalComplianceIntegration:
    ...
    def test_gdpr_provider_filtering(self):
        ...
        with patch.object(registry._region_detector, 'detect_region', return_value='DE'), \
             patch('src.support_deflect_bot.utils.region_detector.is_gdpr_region', return_value=True):
            
            # Get providers for EU region
            available_providers = registry.get_available_providers(ProviderType.LLM, region='DE')
            ...
            if non_compliant_count > 0:
                logger.warning(f"Found {non_compliant_count} non-GDPR providers in EU region")
            
            logger.info(f"EU region: {gdpr_compliant_count} GDPR compliant, {non_compliant_count} non-compliant")
...
class TestErrorHandlingIntegration:
    ...
    def test_no_api_keys_graceful_degradation(self):
        ...
        from src.support_deflect_bot.core.providers.config import ProviderRegistry, ProviderSelector
        from src.support_deflect_bot.core.providers.implementations import register_all_providers
        from src.support_deflect_bot.core.providers.base import ProviderType
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            fallbacks = selector.select_with_fallbacks(ProviderType.LLM, max_fallbacks=5)
            
            # May have some providers (like Ollama or Claude Code) that don't require API keys
            logger.info(f"No API keys: Primary={primary is not None}, Fallbacks={len(fallbacks)}")
    ...
    def test_provider_failure_handling(self):
        ...
        from src.support_deflect_bot.core.providers.config import ProviderRegistry
        from src.support_deflect_bot.core.providers.base import ProviderType, ProviderUnavailableError
        ...
        logger.info(f"Handled provider failure gracefully, {len(available)} providers remain")
...
class TestRealProviderIntegration:
    ...
    def test_openai_real_integration(self):
        ...
        from src.support_deflect_bot.core.providers.implementations import OpenAIProvider
        ...
        logger.info(f"✅ OpenAI integration test passed: {health['response_time_ms']:.1f}ms")
    ...
    def test_groq_real_integration(self):
        ...
        logger.info(f"✅ Groq integration test passed: {health['response_time_ms']:.1f}ms")
    ...
    def test_claude_api_real_integration(self):
        ...
        from src.support_deflect_bot.core.providers.implementations import ClaudeAPIProvider
        
        provider = ClaudeAPIProvider(api_key=os.getenv('ANTHROPIC_API_KEY'))
        
        # Test availability
        assert provider.is_available(), "Claude API should be available with valid API key"
        
        # Test health check
        health = provider.health_check()
        assert health['status'] == 'healthy', f"Claude API health check failed: {health}"
        ...
        logger.info(f"✅ Claude API integration test passed: {health['response_time_ms']:.1f}ms")
    
    def test_claude_code_integration(self):
        """Test Claude Code provider integration (no API key required)."""
        from src.support_deflect_bot.core.providers.implementations import ClaudeCodeProvider
    ...
    @pytest.mark.skipif(
        not any([os.getenv(key) for key in ['OPENAI_API_KEY', 'GROQ_API_KEY', 'ANTHROPIC_API_KEY']]),
    ...
    def test_multi_provider_real_fallback(self):
        ...
        for i, provider in enumerate(fallbacks):
            ...
            logger.info(f"Fallback {i+1}: {config.name} - {'✅ Available' if is_available else '❌ Unavailable'}")
            
            if is_available:
                health = provider.health_check()
                assert health['status'] in ['healthy', 'degraded'], f"{config.name} health check failed"
